...
<form name="myform" action="<?php echo U('Global/role_priv');?>" method="post">
...
<tbody>
   <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
    <td align="left">
     <input name="menuid[]" value="<?php echo ($vo["id"]); ?>" level="<?php if($vo['fid']==0){ echo '0';}else{echo '1';}?>"  onclick="javascript:checknode(this);" type="checkbox" <?php if($vo['flag']==1){ echo 'checked';}?>></td>
    <td class="tdleft"><?php echo ($vo["space"]); echo ($vo["cname"]); ?>
	<font color="blue"><?php if(($vo["hidden"]) == "1"): ?>[隐藏]-<?php echo ($vo["module_alias"]); else: ?>(<?php echo ($vo["module_alias"]); ?>)<?php endif; ?></font>
	</td>
  </tr><?php endforeach; endif; else: echo "" ;endif; ?> 
</tbody>
</table>
<input value="<?php echo ($roleid); ?>" name="roleid" type="hidden">
