...
<form action="<?php echo U('Global/manager_edit');?>" method="post" id="myform" >
<table class="table_form contentWrap" width="100%">
<tbody><tr>
<td>用户名</td> 
<td><input name="username" value="<?php echo ($info["username"]); ?>"  disabled="disabled"class="input-text" id="username" type="text"></td>
</tr>
<tr>
<td>密码</td>
...
<tr>
<td>email</td>
<td><input name="email" value="<?php echo ($info["email"]); ?>" class="input-text" id="email" type="text"></td>
</tr>
<tr>
<td>所属角色</td>
<td>
<select name="roleid" id="roleid">
<?php if(is_array($role_list)): $i = 0; $__LIST__ = $role_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["roleid"]); ?>"  <?php if($vo['roleid']==$info['roleid']){echo "selected";} ?> ><?php echo ($vo["rolename"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
 ...
 <tr>
    <td>状态</td>
    <td >
    <input name="status" value="0" type="radio" <?php if(($info["status"]) == "0"): ?>checked<?php endif; ?>> 开启&nbsp;&nbsp;&nbsp;&nbsp;
	<input name="status" value="1" type="radio" <?php if(($info["status"]) == "1"): ?>checked<?php endif; ?>> 关闭
</td>
  </tr>
</tbody></table>
</div>
